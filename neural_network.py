...
model.compile(optimizer='SGD', 			# adam is a type of stochastic gradient descent 
              loss=loss_fn,					# the loss is our loss function 
              metrics=['accuracy'])

# Let us train the model
model.fit(x_train, y_train, epochs=3)		# epochs are the numnbers of trainings (each train goes through the entire trainins set)
